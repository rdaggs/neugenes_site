...
def process(base_dir,
            ...
            z_threshold=1.2):
    # preprocess dataset 
    if(structure_acronymns == "FULL_BRAIN"): structure_acronymns = config.FULL_BRAIN
    base_dir = os.path.join(os.path.dirname(config.root_directory),base_dir)
    spaces_in_filenames(base_dir)
    R  = nd2_to_tiff(base_dir,layer_in_tiff)
    print('layer in tiff=1')
    ...
    for filename in os.listdir(data_path):
        ...
        if os.path.isfile(path) and path.lower().endswith(('.jpeg', '.tif', '.jpg', '.png')):
            ...
            cell_yx = process_image(path,dot_count,expression_intensity,threshold_scale,
                                    patch_size, ring_width, z_threshold)
            ...
            for i in range(len(structures)): # iteratively process structures 
                ...
                # print('cell',len(cell_yx),'mask',len(mask_yx))
